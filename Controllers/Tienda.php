	...
	class Tienda extends Controllers{
		...
		public function delCarrito(){
			if($_POST){
				...
				if(is_numeric($idproducto) and ($option == 1 or $option == 2)){
					...
					// Reordenar indice de array eliminado
					sort($arrCarrito);
					$_SESSION['arrCarrito'] = $arrCarrito;
					...
					if (empty($_POST['datapay'])) {
						...
						if($request_pedido > 0){
							...
							foreach ($_SESSION['arrCarrito'] as $producto) {
								...
								$this->insertDetallePedido($request_pedido,$productoid,$precio,$cantidad);
							}
							...
							$arrResponse = array('status' => true, "orden" => $orden, "transaccion" => $transaccion, 'msg' => 'Pedido realizado.');
		...
		public function contacto(){
			if($_POST){
				...
				if($userContact > 0){
					$arrResponse = array('status' => true, 'msg' => "Mensaje enviado correctamente.");
					//Enviar correo
