<?php
  ...
  class Pedidos extends Controllers{
    ...
    public function pedidos(){
      ...
      $data['page_tag'] = "Pedidos - ".NOMBRE_EMPRESA;
      ...
      if($_SESSION['permisosMod']['r']){
        ...
        for ($i=0; $i < count($arrData); $i++) {
          ...
          if ($_SESSION['permisosMod']['r']) {
            ...
            <a title="Generar PDF" href="'.base_url().'/factura/generarFactura/'.$arrData[$i]['idpedido'].'" target="_blank" class="btn btn-danger btn-sm"><i class="fas fa-file-pdf"></i></a> ';

            if ($arrData[$i]['idtipopago'] ==1) {
              $btnView .= '<a title="Ver Transacción" href="'.base_url().'/pedidos/transaccion/'.$arrData[$i]['idtransaccionpaypal'].'" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-paypal" aria-hidden="true"></i></a>';
            }else{
              $btnView .= '<button class="btn btn-secondary btn-sm" disabled=""><i class="fa fa-paypal" aria-hidden="true"></i></button>';
            }
          }
          if ($_SESSION['permisosMod']['u']) {
            $btnEdit = '<button class="btn btn-primary btn-sm btnEditPedido" onClick="fntEditPedido(this, '.$arrData[$i]['idpedido'].')" title="Editar Pedido"><i class="fas fa-pencil-alt"></i></button>';
          }
          // if ($_SESSION['permisosMod']['d']) {
          //   $btnDelete = '<button class="btn btn-danger btn-sm btnDeletePedido" onClick="fntDeletePedido('.$arrData[$i]['idpedido'].')" title="Eliminar Pedido"><i class="fas fa-trash-alt"></i></button>';
          // }
    ...
    public function transaccion($transaccion){
      ...
      $data['page_tag'] = "Detalles de la transacción";
      $data['page_title'] = "Detalles de la transacción - Tienda Virtual";
      $data['page_name'] = "detalle_transaccion";
      $data['page_functions_js'] = "functions_pedidos.js";
      $data['objTransaccion'] = $requestTransaccion;
      // $data['arrPedido'] = $this->model->selectPedido(4,$idpersona);
