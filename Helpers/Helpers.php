<?php
    ...
    //Envio de correos Mailer y Local
    function sendEmail($data,$template){
        if(ENVIRONMENT == 1){
            $asunto = $data['asunto'];
            $emailDestino = $data['email'];
    ...
    // Obtiene datos del usuario en session
	...
	// Elimina exceso de espacios entre palabras (Inyecciones SQL)
    ...
    function getPageRout(string $ruta){
        require_once ("Libraries/Core/Mysql.php");
        $con = new Mysql();
        $sql = "SELECT * FROM pagina WHERE ruta = '{$ruta}' AND status != 0";
        $request = $con->select($sql);
        if (!empty($request)) {
            $request['portada'] = $request['portada'] != "" ? media()."/img/uploads/".$request['portada'] : "";
        }
        return $request;
    }
    ...
    function viewPage(int $idpagina){
        ...
        $sql = "SELECT * FROM pagina WHERE idpagina = $idpagina";
        $request = $con->select($sql);
        if (($request['status'] == 2 && isset($_SESSION['permisosMod']) && $_SESSION['permisosMod']['u'] == true) OR $request['status'] == 1) {
			return true;
		}else{
            return false;
        }
