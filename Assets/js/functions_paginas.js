...
tablePaginas = $("#tablePaginas").DataTable({
  ...
  ajax: {
    url:
      base_url +
      "/Paginas/getPaginas" /* Ruta a la funcion getRoles que esta en el controlador roles.php*/,
    dataSrc: "",
  },
  columns: [
    { data: "idpagina" },
...
// Del Form
function fntDeleteInfo(idPagina) {
  swal(
    {
    ...
    function (isConfirm) {
      if (isConfirm) {
        let request = window.XMLHttpRequest
          ? new XMLHttpRequest()
          : new ActiveXObject("Microsoft.XMLHTTP");
        let ajaxUrl = base_url + "/Paginas/deletePagina";
        ...
        request.onreadystatechange = function () {
          if (request.readyState == 4 && request.status == 200) {
            ...
            if (objData.status) {
              ...
              $("#tablePaginas").DataTable().ajax.reload();
            } else {
