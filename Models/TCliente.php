<?php
	...
	trait TCliente{
		...
		public function insertDetallePedido(int $idpedido, int $productoid, float $precio, int $cantidad){
			$this->con = new Mysql();
		...
		public function setSuscripcion(string $nombre, string $email){
			$this->con = new Mysql();
			$sql = 	"SELECT * FROM suscripcion WHERE email = '{$email}'";
			$request = $this->con->select_all($sql);
			if(empty($request)){
				$query_insert  = "INSERT INTO suscripcion(nombre,email) 
									VALUES(?,?)";
