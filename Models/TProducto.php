<?php
	...
	Trait TProducto{
			...
			$sql = "SELECT p.idproducto,
					...
					if(!empty($request)){
						...
						if(count($arrImg) > 0){
							...
							$arrImg[0]['url_image'] = media().'/img/uploads/product.png';
						}
		...
		public function cantProdSearch($busqueda){
			...
			$sql = "SELECT COUNT(*) as total_registro 
							FROM producto 
							WHERE nombre LIKE '%$busqueda%' AND status = 1 ";
			$result_register = $this->con->select($sql);
			$total_registro = $result_register;
			return $total_registro;
		}
		...
		public function getProdSearch($busqueda, $desde, $porpagina){
			...
			$sql = "SELECT p.idproducto,
			 		...
			 		INNER JOIN categoria c
			 		ON p.categoriaid = c.idcategoria
					WHERE p.status != 0 AND p.nombre LIKE '%$busqueda%' ORDER BY p.idproducto DESC Limit $desde,$porpagina";
